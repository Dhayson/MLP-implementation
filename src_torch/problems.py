...
from src_torch.model import eval_model, train_model
import torch
...
def classification_problem_torch(
    mlp: nn.Module,
    ...
    while True:
        ...
        if show_each_n_steps != -1 and t % show_each_n_steps == 0:
            print(f"t = {t}")
        ...
        for _i in range(detail):
            train_loss = train_model(mlp, loss, optimizer, features_train, labels_train, sample=gradient_type[0], n=gradient_type[1], device=device)
            t += 1
            
    train_loss, train_accuracy = eval_model(mlp, loss, features_train, labels_train, kind="Classification", device=device)
    print(f"train loss: {train_loss} train accuracy: {train_accuracy}")
    val_loss, val_accuracy = eval_model(mlp, loss, features_val, labels_val, kind="Classification", device=device)
